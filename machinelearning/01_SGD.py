"""
    ...
    模型构建：模型 线性模型 ∑(Y -θ.T*X)
              策略 经验极小化
              算法 随机梯度下降 一个样本更新一次θ向量θ=θ+α* gd
    模型评估：R^2 = 1- RSS/TSS
    测试手写SGD算法与scikit-learn框架里面的最小二乘算法结果
    画图展示线性回归模型，在这个问题上最小二乘效果要好
"""
...
def fit(X, Y, alpha=0.01, max_iter=100, fit_intercept=True, tol=1e-4):
    ...
    for i in range(max_iter):
        ...
        for k in range(m):
            # 1、计算出当前样本实际值与预测值之间的差值
            y_true = Y[k][0]
            y_predict = predict(X[k], theta, intercept)
            diff = y_true - y_predict
            # 2、开始更新所有的θ值（向量）
            for j in range(n):
                theta[j] += alpha * diff * X[k][j]
            # 3、如果有截距 更新截距
            if fit_intercept:
                intercept += alpha * diff * 1
